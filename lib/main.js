		...
		if ( isNonNegativeInteger( N ) ) {
			...
			if ( hasOwnProp( N, 'round' ) ) {
				if ( !contains( ROUNDING_MODES, N.round ) ) {
					throw new TypeError( format( '0CG4E', N.round ) );
				}
				opts.round = N.round;
			}
		} else {
			throw new TypeError( format( '0CG4F', N ) );
		}
	} else if ( arguments.length > 3 ) {
		if ( !isNonNegativeInteger( N ) ) {
			throw new TypeError( format( '0CG2y', N ) );
		}
		n = N;
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '0CG4G', options ) );
		}
		if ( hasOwnProp( options, 'round' ) ) {
			if ( !contains( ROUNDING_MODES, options.round ) ) {
				throw new TypeError( format( '0CG4E', options.round ) );
			}
