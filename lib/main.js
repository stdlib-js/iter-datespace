		...
		if ( isNonNegativeInteger( N ) ) {
			...
			if ( hasOwnProp( N, 'round' ) ) {
				if ( !contains( ROUNDING_MODES, N.round ) ) {
					throw new TypeError( format( '0Oh41,Lv', N.round ) );
				}
				opts.round = N.round;
			}
		} else {
			throw new TypeError( format( '0Oh42,Lw', N ) );
		}
	} else if ( arguments.length > 3 ) {
		if ( !isNonNegativeInteger( N ) ) {
			throw new TypeError( format( '0Oh2m,L9', N ) );
		}
		n = N;
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '0Oh43,Lp', options ) );
		}
		if ( hasOwnProp( options, 'round' ) ) {
			if ( !contains( ROUNDING_MODES, options.round ) ) {
				throw new TypeError( format( '0Oh41,Lv', options.round ) );
			}
